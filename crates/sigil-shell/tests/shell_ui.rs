...
struct Stub {
    ...
    /// Links this stub took, in order. See `follow`.
    followed: std::rc::Rc<std::cell::RefCell<Vec<sigil::Link>>>,
    /// Whether it takes them at all.
    takes_links: bool,
...
impl App for Stub {
    ...
    /// A `sigil://` link, if this stub takes them. Recorded rather than
    /// acted on, which is all a test needs to see.
    fn follow(&mut self, _ctx: &mut AppContext<'_>, link: &sigil::Link) -> bool {
        if !self.takes_links {
            return false;
        }
        self.followed.borrow_mut().push(link.clone());
        true
    }
...
fn a_sigil_link_is_asked_about_before_anything_happens() {
    ...
    sigil_platform::deeplink::offer("sigil://room/TestRoomSecretNotARea1RoomDoNotUseAAAAAAAAAA")
        .expect("a good link");
    let (mut h, took) = shell_with(Stub {
        takes_links: true,
        ..Stub::named("Calls", 0)
    });
    h.run();
